...
function nodeIsOnlyUsedInTypePosition(
...
function generateImportDeclarationDocumentation(
  ...
  for (const namedImport of importDeclaration.getNamedImports() ?? []) {
    ...
    if (
      ...
      nodeIsOnlyUsedInTypePosition(aliasNode || namedImport.getNameNode())
    ) {
...
function generateParameterDocumentation(
  ...
  for (const param of params) {
    const paramType =
      ...
      param
        .getType()
        .getText(param, TypeFormatFlags.UseAliasDefinedOutsideCurrentScope);
...
function generateReturnTypeDocumentation(
  ...
  const functionReturnType =
    functionNode.getReturnTypeNode()?.getText() ||
    functionNode
      .getReturnType()
      .getText(
...
function generateConstructorDocumentation(
...
function generateClassMemberDocumentation(
  ...
  if (Node.isObjectProperty(classMemberNode))
    generateInitializerDocumentation(classMemberNode);
  if (Node.isGetAccessorDeclaration(classMemberNode))
    generateGetterDocumentation(classMemberNode);
  if (Node.isSetAccessorDeclaration(classMemberNode))
    generateSetterDocumentation(classMemberNode);
  if (Node.isConstructorDeclaration(classMemberNode))
    generateConstructorDocumentation(classMemberNode);
  if (Node.isMethodDeclaration(classMemberNode))
...
function generateTypedefDocumentation(typeAlias: TypeAliasDeclaration): string {
  ...
  const isObjectType =
    Node.isTypeLiteral(typeNode) && typeAlias.getType().isObject();
  ...
  const jsDoc =
    properties.length || typeParams.length
...
function generateObjectPropertyDocumentation(
  ...
  const existingPropDocs =
    node.getJsDocs()?.[0]?.getDescription()?.trim() || "";
...
function generateTopLevelVariableDocumentation(varNode: VariableDeclaration) {
  const paramType =
    varNode.getTypeNode()?.getText() ||
    varNode
      .getType()
      .getText(varNode, TypeFormatFlags.UseAliasDefinedOutsideCurrentScope);
  if (!paramType) {
    return;
  }
  ...
  const constTag = tags.find((tag) =>
    ["const", "constant"].includes(tag.getTagName())
  );
...
function generateDocumentationForSourceFile(sourceFile: SourceFile): void {
  ...
  const importDeclarations = sourceFile
    ...
    .map((declaration) =>
      generateImportDeclarationDocumentation(declaration).trim()
    )
    .join("\n")
    .trim();
  ...
  sourceFile.insertText(
    sourceFile.getFullText().length - 1,
    `\n\n${interfaces}`
  );
...
export function transpileFile({
  ...
  const sourceFile = project.createSourceFile(`index.tsx`, code);
